...
const Ticket = () => {
    ...
    const { register, handleSubmit, getValues, reset } = useForm({ resolver: zodResolver(addMessage), defaultValues: { ticketId: data?.id as string, content: "", userId: session.data?.user?.id as string } });
    return (
        <>
                ...
                <ul className="mx-auto w-4/6 mb-10">
                    {data?.messages?.map((message) => (
                        <li key={message.id} className="m-4">
                            <div className={`flex flex-col relative w-3/6 ${session.data?.user?.id === message.userId ? ("ml-auto") : ("mr-auto")} bg-base-300 rounded-lg p-4`}>
                                <div className="mb-4">
