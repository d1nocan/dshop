      ...
      <div className="navbar bg-neutral text-neutral-content w-11/12 mx-auto rounded-b-xl">
        <div className="flex-1 gap-3">
          ...
          <label className="swap swap-rotate">
            ...
            <Sun className="swap-on fill-current w-10 h-10 pt-3" data-set-theme="dark" data-act-class="ACTIVECLASS" />

            <Moon className="swap-off fill-current w-10 h-10 pt-3" data-set-theme="light" data-act-class="ACTIVECLASS" />
          </label>
        </div>
        ...
        {session?.user ? (
          <div className="dropdown dropdown-hover dropdown-end pr-2">
            <label tabIndex={0} className="btn btn-ghost btn-circle avatar">
              <div className="w-8">
                <Image alt={session?.user?.name as string} src={session?.user?.image as string} layout="fill" className="rounded-full" />
              </div>
            </label>
            <ul tabIndex={0} className="p-2 gap-3 shadow menu menu-compact dropdown-content bg-base-200 rounded-box w-52">
              <li>
